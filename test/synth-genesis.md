# Example: Launching $SYNTH — Fictional Example Token

> ⚠️ **Disclaimer**: `$SYNTH` is a **TEST  fictional token**

## Narrative context

A team of developers wants to launch a new on-chain AI agent called **SYNTH** (symbol: `$SYNTH`), using the Virtual Protocol infrastructure as an architectural reference.

### Test actors
- **John** — founder, applies to create SYNTH
- **Giulia** — staker, locks VIRTUAL to gain voting power
- **Validator1** — validator who approves technical contributions
- **Contributor1** — developer who proposes an improvement to SYNTH's model
...
CA: 0x819b03707078d192fda282768f872e0f7daf0dd7

### Step 1 — Application (Genesis)
John sends `100 VIRTUAL` as a deposit and calls `AgentFactory.proposeAgent()` to register SYNTH, specifying name, symbol (`$SYNTH`), and the core's initial parameters.
...
1. Clones `AgentToken` → creates `$SYNTH`
2. Clones `AgentDAO` → creates SYNTH's specific DAO
3. Mints an `AgentNft` representing the agent
4. John stakes his VIRTUAL and receives `$SYNTH` in return
5. A **TBA (Token Bound Account)** is created and linked to the AgentNft

**Check**: `AgentNft.ownerOf(tokenId)` returns the correct address, `$SYNTH.totalSupply()` > 0.
...
**Check**: she receives `$SYNTH` proportionally, and her voting power is correctly delegated.

### Step 5 — Contribution proposal
Contributor1 proposes an improvement to SYNTH's dataset on `AgentDAO` (action = `ServiceNft.mint`). After approval, they mint a `ContributionNft`, verifying that the caller is indeed the proposer.

### Step 6 — Core upgrade
Validator1 votes on the proposal on `AgentDAO`. Once executed:
- A `ServiceNft` is minted
- SYNTH's **maturity score** is updated
...
|---|---|---|
| AgentFactory | `proposeAgent()` | Registers a new application for the agent (SYNTH) |
...
## Example contract ($SYNTH — fictional)
...
// $SYNTH is not a real token and is not affiliated with Virtual Protocol
...
contract SynthToken {
    // ---- Metadata ----
    string public name = "Synthetic Token";
    string public symbol = "SYNTH";
...
- This file can be converted into a Mocha/Hardhat test suite (`describe("SYNTH lifecycle")`) following the step order above, run first against a local fork of Robinhood Chain before ever touching mainnet directly.
