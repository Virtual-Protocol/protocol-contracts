...
async function main() {
  try {
    ...
    const beSellWallWallet = process.env.BE_SELL_WALL_WALLET;
    if (!beSellWallWallet) {
      throw new Error("BE_SELL_WALL_WALLET not set in environment");
    }
    ...
    const tx2 = await multicall3.grantAdmin(beSellWallWallet);
    await tx2.wait();
    console.log("✅ Admin role granted to:", beSellWallWallet);
    console.log("✅ beOpsWallet Is admin:", await multicall3.isAdmin(beSellWallWallet));
